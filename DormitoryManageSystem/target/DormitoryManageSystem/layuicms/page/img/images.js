layui.config({
        	...
        	$.ajax({
                ...
                setTimeout(function(){
                    for(var i=imgNums*(page-1); i<maxPage; i++){
                        imgList.push('<li><img width="200px" height="300px" src="'+ data[i].imgSrc +'"><input type="hidden" value="'+data[i].imgSrc+'"><div class="operate"><div class="check"><input type="checkbox" name="belle" lay-filter="choose" lay-skin="primary" title="'+data[i].imgTitle+'"></div><i class="layui-icon img_del">&#xe640;</i></div></li>')
                    }
    ...
    $("body").on("click",".img_del",function(){
        var _this = $(this);
        //需要找出路径
        var SRC = $(this).parents("li").find("input[type=hidden]").val();
        layer.confirm('确定删除图片"'+_this.siblings().find("input").attr("title")+'"吗？',{icon:3, title:'提示信息'},function(index){
            ...
            //删除单张图片，需要找出路径
            var src = SRC.substr(22,SRC.length-22);
            $.ajax({
		 		url:"../../../../userheadinit",
		 		type:"post",
		 		data:{src:src},
