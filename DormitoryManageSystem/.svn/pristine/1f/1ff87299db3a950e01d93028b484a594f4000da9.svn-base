...
/*        oldPwd : function(value, item){
    ...
    form.on("submit(changeUser)",function(data){
        ...
        var readyname=$("#readyname").val();
		var sex=$("#sex").val();
		var phone=$("#phone").val(); 
		var birthday=$("#birthday").val();
		var province1=data.field.province;
		...
		for(var i=0;i<like.length;i++){
			if(like[i].checked){
				chec=like[i].value;
				allchecked=allchecked+","+chec;
			}
		}
		var Allchecked=allchecked.substring(1, allchecked.length);
		var email=$("#email").val();
        $.ajax({
        	url:"../../../../Manager",
        	type:"post",
