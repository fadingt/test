...
//			}
	...
	$("body").on("click",".links_edit",function(){  //编辑
		//layer.alert('暂时没有修改公告的权限，敬请谅解',{icon:6, title:'公告编辑'});
        ...
        for(var i=0;i<linksData.length;i++){
            if(linksData[i].linksId == _this.attr("data-id")){
                //alert(linksData[i].linksId);

                var index = layui.layer.open({
                    title : "编辑公告",
                    type : 2,
                    content : "linksEdit.html?id="+linksData[i].linksId,
                    success : function(layero, index){
                        // setTimeout(function(){
                        // 	layui.layer.tips('点击此处返回友链列表', '.layui-layer-setwin .layui-layer-close', {
                        // 		tips: 3
	...
	$("body").on("click",".links_del",function(){  //删除
		...
		/*	$.ajax({
			...
			for(var i=0;i<linksData.length;i++){
				if(linksData[i].linksId == _this.attr("data-id")){
					//alert("所删除id："+linksData[i].linksId);
					$.ajax({
						url : "../../../notice/del",
						type : "post",
						data:{
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	//+'<td><a style="color:#1E9FFF;" target="_blank" href="'+currData[i].linksUrl+'">'+currData[i].linksUrl+'</a></td>'
                        +'<td>'+currData[i].linksUrl+'</td>'
			    	+'<td>'+currData[i].masterEmail+'</td>'
			    	+'<td>'+currData[i].linksTime+'</td>'
			    	+'<td>'+currData[i].showAddress+'</td>'
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-mini links_edit" data-id="'+data[i+nums*(curr-1)].linksId+'"><i class="iconfont icon-edit" ></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-mini links_del" data-id="'+data[i+nums*(curr-1)].linksId+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
