layui.config({
                ...
                $.ajax({
                    ...
                    success : function(data){
                        ...
                        for(var i=0;i<newsData.length;i++){
                            ...
                            if(newsStr.newsName.indexOf(selectStr)>-1 || newsStr.newsAuthor.indexOf(selectStr)>-1  || newsStr.newsTime.indexOf(selectStr)>-1){
    ...
    $("body").on("click","#edit",function(){  //编辑
        ...
        /*location.href("newsadd.html");*/
        var _this = $(this);
            //_this.parents("tr").remove();
            for(var i=0;i<newsData.length;i++){
                if(newsData[i].newsId == _this.attr("data-id")){
                    //alert(newsData[i].newsId);

                    $.jq_Panel({
                        title: "编辑处分",
                        url: "../../../editCriticize.html?id="+newsData[i].newsId,
                        iframeWidth: 800,
                        iframeHeight: 600,})
    ...
    $("body").on("click","#add",function(){  //编辑
        /*layer.alert('您点击了文章编辑按钮，由于是纯静态页面，所以暂时不存在编辑内容，后期会添加，敬请谅解。。。',{icon:6, title:'文章编辑'});*/
        /*location.href("newsadd.html");*/
        $.jq_Panel({
            title: "添加处分",
            url: "../../../addCriticize.html",
            iframeWidth: 800,
            iframeHeight: 600,})
        //window.location.href="../../../addUser.html";
    })
            ...
            if(currData.length != 0){
                for(var i=0;i<currData.length;i++){
                    ...
                    dataHtml +='<td>'+currData[i].newsTime+'</td>'
                        ...
                        +'<td>'
                        +  '<a class="layui-btn layui-btn-mini news_edit" id="edit" data-id="'+data[i+nums*(curr-1)].newsId+'"><i class="iconfont icon-edit"></i> 编辑</a>'
                        /* +  '<a class="layui-btn layui-btn-normal layui-btn-mini news_collect"><i class="layui-icon">&#xe600;</i> 收藏</a>'*/
                        +  '<a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="'+data[i+nums*(curr-1)].newsId+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
                        +'</td>'
                        +'</tr>';
                }
            }else{
